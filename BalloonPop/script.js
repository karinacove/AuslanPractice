...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function getValidNumbersForColour(colour) {
    ...
    const colourOffsets = {
      green: 41, red: 41, orange: 43, yellow: 42, purple: 43, pink: 44, blue: 44, brown: 42, black: 44, white: 43
    };
    let extendedNumbers = [];
    const start = colourOffsets[colour];
    if (start !== undefined) {
      for (let i = start; i <= 100; i += 4) extendedNumbers.push(i);
    }
  ...
  function endGame(early = false) {
    ...
    if(score >= 120) continueBtn.style.display = 'none';
    else continueBtn.style.display = 'inline';
  ...
  finishBtn.addEventListener('click', () => { endModal.style.display='none'; });
