...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const menuBtn = document.getElementById("menu-btn"); // we will not show this in final modal per request
  ...
  function showEndMenuModal() {
    ...
    logoutImg.style.width = "120px";
    logoutImg.style.height = "120px";
    logoutImg.style.cursor = "pointer";
    logoutImg.addEventListener("click", async () => {
      try { await submitFinalResultsToForm(); } catch (err) { console.warn("Submit failed:", err); }
      clearProgress(true); // clear student info too
      window.location.href = "../index.html";
    });
