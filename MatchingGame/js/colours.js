document.addEventListener("DOMContentLoaded", function () {
  ...
  Object.assign(feedbackImage.style, { position:"fixed", top:"50%", left:"50%", transform:"translate(-50%, -50%)", width:"200px", display:"none", zIndex:"1000" });
  ...
  function saveProgress() {
    const savedData = { currentLevel, currentPage, currentColours, levelAttempts: levelAttempts.map(lvl=>({ correct: Array.from(lvl.correct), incorrect: lvl.incorrect })), startTime };
  ...
  function touchStart(e) {
    ...
    const handleEnd=ev=>{
      ...
      if(el&&el.classList.contains("slot")) drop({ preventDefault:()=>{}, dataTransfer:{getData:k=>k==="text/plain"?colour:src}, currentTarget:el });
  ...
  function loadPage() {
    ...
    levelTitle.innerText=`Level ${currentLevel+1}: `+(mode==="signToImage"?"Match the Sign to the Picture":mode==="imageToSign"?"Match the Picture to the Sign":"Match Signs and Pictures (Mixed)");

    if(currentPage===0 && currentColours.length===0){
      const shuffled=shuffle([...allColours]);
      currentColours=[]; for(let i=0;i<pagesPerLevel;i++) currentColours.push(shuffle(shuffled.slice(i*5,(i+1)*5)));
    }
    ...
    pageColours.forEach(colour=>{
      ...
      slot.className="slot"; slot.dataset.letter=colour;
    ...
    draggableColours.forEach((colour,i)=>{
      ...
      img.className="draggable"; img.draggable=true; img.dataset.letter=colour;
