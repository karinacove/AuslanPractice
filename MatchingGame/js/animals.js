...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // ----------------------------
  const SAVE_KEY = "animalsGameSave_v1";
  ...
  const levelDefinitions = [
    { words: wordBanks[1], pages: 3, name: "Pets and Farm" },
    { words: wordBanks[2], pages: 3, name: "Zoo Animals" },
    { words: wordBanks[3], pages: 3, name: "Austrlian Animals" },  
    { words: wordBanks[4], pages: 3, name: "Water Animals" }
  ];
  ...
  function buildGridForPage(pageWords, pageIdx) {
    ...
    pageWords.forEach(word => {
      ...
      const url = `assets/animals/${slotType === "sign" ? "signs" : "clipart"}/${word}.png`;
  ...
  function buildDraggablesForPage(info, pageWords, gridType) {
    ...
    draggablesToUse.forEach((word, idx) => {
      ...
      img.src = `assets/animals/${folder}/${word}.png`;
