/* ============================
   ...
   Fully merged: dropzones, again/check logic, Level-specific draggables
   ============================ */
...
function buildPromptForCurrentQuestion() {
  ...
  if (currentLevel <= 4) {
    ...
    if (currentLevel === 1) combos.push(`${currentSentence.animal}-${currentSentence.number}`);
      ...
      combos.push(`${currentSentence.animal}-${currentSentence.number}`, `${currentSentence.food}-${currentSentence.colour}`);
    }
    combos.forEach(c => {
      const img = document.createElement("img");
      img.src = compositeImagePath(c); img.alt = c; img.className = "promptImage";
      imageDiv.appendChild(img);
    });
  } else if (currentLevel === 5) {
    if (currentSentence.video) {
      const vid = document.createElement("video");
      vid.controls = true; vid.width = 320;
...
function buildAnswerDropzones() {
  ...
  if (currentLevel === 1 || currentLevel === 2 || currentLevel === 3) {
    dropCount = [0,2,4,6,8].includes(roundInLevel) ? 2 : 1;
  } else if (currentLevel === 4) {
    dropCount = [0,2,4,6,8].includes(roundInLevel) ? 5 : 3;
  } else if (currentLevel === 5) {
    dropCount = 3;
...
function dropHandler(e) {
  ...
  const img = document.createElement("img");
  img.className = "droppedImage"; img.alt = value;
  ...
  const allFilled = dropzones.every(d => d.dataset.filled && d.dataset.filled.length > 0);
...
function buildDraggablesForCurrentQuestion() {
  ...
  if (currentLevel === 1) {
    if (signsDraggable) items = addDecoys([currentSentence.animal,currentSentence.number],[...allAnimals,...allNumbers],10);
    else items = addDecoys([`${currentSentence.animal}-${currentSentence.number}`],allAnimalNumberCombos,8);
  } else if (currentLevel === 2) {
    if (signsDraggable) items = addDecoys([currentSentence.food,currentSentence.colour],[...allFoods,...allColours],10);
    else items = addDecoys([`${currentSentence.food}-${currentSentence.colour}`],allFoodColourCombos,8);
  } else if (currentLevel === 3 || currentLevel === 4) {
    if (signsDraggable) items = addDecoys([currentSentence.animal,currentSentence.number,currentSentence.food,currentSentence.colour],[...allAnimals,...allNumbers,...allFoods,...allColours],12);
    else items = addDecoys([`${currentSentence.animal}-${currentSentence.number}`,`${currentSentence.food}-${currentSentence.colour}`],allPairCombos,8);
  } else if (currentLevel === 5) {
    items = Array.isArray(currentSentence.videoSigns) ? currentSentence.videoSigns.slice() : [];
    items = addDecoys(items,allVideoSigns,8);
  }

  items.forEach(word => {
    const div = document.createElement("div");
    div.className = "draggable"; div.draggable = true; div.dataset.value = word;
    const img = document.createElement("img"); img.alt = word; img.className = "draggableImage";
...
function buildQuestion() {
  ...
  if (currentLevel === 5) {
    ...
    for (let i = 0; i < 3; i++) {
      const dz = document.createElement("div");
      dz.className = "dropzone"; dz.dataset.expected="sign"; dz.dataset.filled="";
      dz.addEventListener("dragover", e=>e.preventDefault());
      dz.addEventListener("drop", dropHandler);
      answerArea.appendChild(dz);
    }
  ...
  if (currentLevel === 6) {
    mode = "record"; expectedDrops = [];
    sentenceDiv.textContent = "Record yourself signing the image.";
    buildPromptForCurrentQuestion();
    startRecordingUI();
    return;
  }
...
async function endGame() {
  ...
  // Optional: submit data to Google Form
  ...
  } catch(err) { console.warn("Google Form submit failed:",err); }
