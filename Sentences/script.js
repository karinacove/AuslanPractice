...
const bottomVerbs = document.getElementById("verbDraggables"); // new container
...
if(!studentName || !studentClass){
  alert("Please log in first."); window.location.href="../index.html";
}else{
...
function signPathFor(word){
  ...
  if(Object.values(VOCAB.helpers).includes(word)) return `assets/signs/helpers/${word}`;
  return "";
}
...
function populateDraggables(level){
  ...
  let leftCount=level<=6?6:9, rightCount=level<=6?6:9;

  // left/right candidates: animals, food, emotions
  ...
  Object.keys(VOCAB.helpers).forEach(h=>{
    bottomVerbs.appendChild(createDraggableNode("helpers",h));
  });
...
function loadProgress(){
  ...
  try{
    ...
    currentLevel=p.currentLevel||1; roundInLevel=p.roundInLevel||0;
    correctCount=p.correctCount||0; incorrectCount=p.incorrectCount||0;
